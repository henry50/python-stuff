...
#webbrowser.open("https://developers.google.com/youtube/v3/getting-started") #Uncomment this line to open the link when the program is run.
        ...
        try:
            ...
            if int(userNum) > 100:
                ...
                if self.videoId  == None:
                    ...
                    self.update()
                    try:
                        self.videoDataRaw = requests.get("https://www.googleapis.com/youtube/v3/videos?part=snippet&id={}&key={}".format(self.videoId, API_KEY)).json()["items"][0]["snippet"]
                    except KeyError:
                        self.urlErr.config(text = "Your API key appears to be invalid", fg = "red")
                    except IndexError:
                        self.urlErr.config(text = "That URL doesn't seem to exist", fg = "red")
                    except:
                        self.urlErr.config(text = "An error occured")
                    else:
                        self.statsDataRaw = requests.get("https://www.googleapis.com/youtube/v3/videos?part=statistics&id={}&key={}".format(self.videoId, API_KEY)).json()["items"][0]["statistics"]
                        self.commentNumber = userNum
                        self.commentDataRaw = requests.get("https://www.googleapis.com/youtube/v3/commentThreads?key={}&textFormat=plainText&part=snippet&videoId={}&maxResults={}&order=relevance".format(API_KEY, self.videoId, self.commentNumber)).json()["items"]
                        self.commentData = []
